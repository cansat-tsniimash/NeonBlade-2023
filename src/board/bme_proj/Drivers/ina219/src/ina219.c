...
static int _write_reg(ina219_t * device, uint8_t reg_addr, uint16_t reg_value)
{
	...
	error = HAL_I2C_Master_Transmit(device->bus, device->address << 1,
			package, sizeof(package), device->timeout);
	return error;
}
...
static int _read_reg(ina219_t * device, uint8_t reg_addr, uint16_t * reg_value)
{
	...
	error = HAL_I2C_Mem_Read(device->bus, device->address << 1,
			reg_addr, 1, (uint8_t*)&raw_reg_value, sizeof(raw_reg_value), device->timeout);
	//if (error)
	//	return error;
